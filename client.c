...
int init_socket_sent(char *host, int port) {
  int sock;
  ...
  if (gethostname == NULL){
    printf("Failing to return the host name\n");
    exit(0);
  }


  memset(&myaddr,0,sizeof(myaddr));
  myaddr.sin_addr=*(struct in_addr*)host_entry->h_addr_list[0];
  myaddr.sin_family=AF_INET;
  myaddr.sin_port=htons(port);

  //printf("Connecting to %s, IP address is %s and port is %d\n", host, inet_ntoa(myaddr.sin_addr),port);
